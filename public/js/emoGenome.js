...
emoViz.emoGenome = function () {
  ...
  //------------------------------------------------------------
  var x, y, yBand, bandHeight = 80,
    genomeWidth = 8,
    colorSchema, animateDuration = 250,
  ...
  function chart(selection) {
    selection.each(function (data) {
      ...
      genomes.enter().append('g')
        ...
        .attr('id', function (d) {
          return 'emogenome_index_' + d.index;
        })
        .attr('centerX', function (d) {
          return x(d.emotions[0].x);
        })
        .attr('centerY', function (d) {
          return y(d.emotions[0].sentiment);
        })
  ...
  function transformData(data) {
    ...
    sliced.forEach(function (ele) {
      var max_score = d3.max(ele.emotions, function (d) {
        return d.score;
      });
  ...
  function emoGenomeRender(selection) {
    ...
    selection.each(function (data, timeIdx) {
      ...
      ynav.append('rect')
        ...
        .attr('y', function (d) {
          return y(data.emotions[0].sentiment);
        })
    ...
    sel.selectAll('.emogenome-bar')
      .each(function (d) {
        d3.select(this)
          ...
          .duration(animateDuration)
          .attr('x', function (d) {
            return x(d.x) - barZoomWidthFactor * genomeWidth / 2;
          })
          .attr('y', function (d) {
            return y(d.center) - barZoomHeightFactor * bandHeight / 2 + barZoomHeightFactor * yBand(d.y0 + d.y);
          })
          .attr('width', barZoomWidthFactor * genomeWidth)
          .attr('height', function (d) {
            return barZoomHeightFactor * yBand(1 - d.y);
          })
          .style('stroke', function (d) {
            return '#aaa';
          });
